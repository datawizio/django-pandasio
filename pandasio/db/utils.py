...
def get_unique_fields(model):
    ...
    unique_fields = (
        unique_fields
        if not isinstance(unique_fields[0], (list, tuple))
        else unique_fields[0]
    )
...
def get_upsert_clause_sql(model, columns=None):
    ...
    upsert_columns = set(columns) - (
        set(get_unique_field_names(model)) | set(get_manage_field_names())
    )
    return ", ".join(
        ['"%(col)s" = EXCLUDED."%(col)s"' % {"col": col} for col in upsert_columns]
    )


def get_dataframe_saver_backend(django_backend):
    mapping = {"django.db.backends.postgresql_psycopg2": "pandasio.db.postgresql"}
